...
;   line.i
;
;   Fast KC85/4 line function, restricted to 256x256 screen area, and
;   max length 127
;
;   hl: x0y0
;   de: x1y1
...
;-------------------------------------------------------------------------------
line:           
                ; check if line_hori or line_vert must be called
...
line_x0x1:
        ...
        ld bc,end_mask  ; lookup final pixal mask and patch code 
        ld a,d      
        and $7
        add c
        ld c,a
...
dy_neg:             ; line goes upward, neg dy and patch code to decrement y-coord
        ...
        ld (p6),a
        jp dy_cont
dy_pos:             ; line goes downward, patch code to increment y-coord
...
p0:     ld a,$ff            ; will be patched with last column
        cp h
        jp nz,cont0
p1:     ld a,$ff            ; will be patched with last y-coord
        cp l 
        jp z,done1
...
done0:
        ...
        inc sp
done1:
        ld a,(hl)       ; write final pixels, anded with final pixel mask 
        xor c
